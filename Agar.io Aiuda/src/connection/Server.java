...
*/
	...
	public void guardarDatos () {
		...
		for (int i = 0; i < game.getPlayers().size(); i++) {
			nombre=game.getPlayers().get(i).getNickName();
			puntaje=game.getPlayers().get(i).getBall().getRadius()+"";
			
			enemigos = "";
			for (int j = 0; j <  game.getPlayers().size(); j++) {
				if(!nombre.equals(game.getPlayers().get(j).getNickName())) {
					enemigos+=game.getPlayers().get(j).getNickName()+",";
				}
			}
			
			if(nombre.equals(game.getWinner().getNickName())) {
				victoria = "si";
			}else {
				victoria = "no";
			}
			
			total += nombre+"-"+puntaje+"-"+fecha+"-"+enemigos+"-"+victoria+"--";
			
			
		}
		
		try {
			String ruta = "./Docs/data.txt";
